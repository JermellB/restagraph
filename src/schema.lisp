...
;;
;; Strategy: scrub the incoming resourcetype definition for duplicate attributes or relationships,
;; and create a new schema-rtypes struct based on the scrubbed definition.
;; Then either add it to the schema as-is, or merge it, depending on whether there's already a
...
(defmethod add-resource-to-schema ((schema hash-table)
  ...
  (let ((existing (resourcetype-exists-p schema (incoming-rtypes-name resourcetype)))
        ...
        (newtype (make-schema-rtypes
                   :name (incoming-rtypes-name resourcetype)
                   :dependent (incoming-rtypes-dependent resourcetype)
                ...
                (if (attribute-exists-p newtype
                    ...
                    (setf (schema-rtypes-attributes newtype)
                          (append (list (make-schema-rtype-attrs
                                          :name (incoming-rtype-attrs-name attr)
                                          :description (incoming-rtype-attrs-description attr)
                                          :values (incoming-rtype-attrs-values attr)))
